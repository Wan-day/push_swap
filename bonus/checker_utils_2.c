...
starting at position *index (which is advanced). On any error, frees what
this function and nums own, then exits with "Error".
*/
static void	fill_from_token(char *token, int *nums, int *index)
{
	char	**words;
	int		i;
	...
	while (words[i])
	{
		if (!str_to_int(words[i], &nums[*index]))
		{
			free_split(words);
			free(nums);
			put_error();
...
int	*parse_tokens(char **tokens, int *count)
{
	...
	*count = count_numbers(tokens);
	...
	while (tokens[i])
	{
		fill_from_token(tokens[i], nums, &index);
		i++;
	}
